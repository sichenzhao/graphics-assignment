...
int gr_mesh_cmd(lua_State* L)
{
    ...
    std::string obj_file_prefix = "Assets/";
	...
	if( i == mesh_map.end() ) {
		mesh = new Mesh( obj_file_prefix + obj_fname );
	} else {
		mesh = i->second;
	}
...
// Set a texture mapping material
extern "C"
int gr_tm_set_material_cmd(lua_State* L)
{
    GRLUA_DEBUG_CALL;
    ...
    luaL_argcheck(L, filename, 2, "Filename expected");
    
    self->setMaterial(material, filename);
...
// Set a transparent
extern "C"
int gr_node_set_mirror_cmd(lua_State* L)
{
    GRLUA_DEBUG_CALL;
...
static const luaL_Reg grlib_node_methods[] = {
  ...
  // for transmission
    ...
    {"set_mirror", gr_node_set_mirror_cmd},
  {0, 0}
};
