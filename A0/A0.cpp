...
//----------------------------------------------------------------------------------------
void A0::uploadUniformsToShader(float move)
{
    m_shader.enable();
    // Query the shader program for the location number of the uniform named "color".
    GLint uniformLocation_colour = m_shader.getUniformLocation("colour");
...
void A0::appLogic()
{
    ...
    m_shape_size = glm::clamp(m_shape_size, 0.1f, 10.0f);
...
void A0::guiLogic()
{
    ...
    ImGui::SliderFloat("Rotation", &m_shape_rotation, 0.0f, 6.283f);
    ImGui::SliderFloat("Size", &m_shape_size, 0.0f, 10.0f);
...
bool A0::keyInputEvent(int key, int action, int mods) {
    ...
    if (action == GLFW_PRESS) {
        if (key == GLFW_KEY_EQUAL) {
            ...
            // increase shape size.
            m_shape_size = m_shape_size + 0.1f;
...
// reset triangle size, rotation and color
